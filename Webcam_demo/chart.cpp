...
void CChart::CalcRect()
{
	...
	offsetY				= m_clientRect.Height()/8 ;
	offsetX				= m_clientRect.Width()/8 ;
	...
	m_axisRect.bottom	= m_clientRect.bottom - (LONG)(offsetY*1.5) ;
}
...
void CChart::Plot(CDC *pDC)
{
	...
	if(m_VerticalLineIndex != -1){

	}else{
		m_VerticalLineIndex = 100;

	}
	pen = new CPen(PS_SOLID,0,RGB(200,10,200));
	old = pDC->SelectObject(pen);
	pDC->MoveTo(Corrdinate(mpSerie[0].dValueX[m_VerticalLineIndex],dRangeY[MIN]));
	pDC->LineTo(Corrdinate(mpSerie[0].dValueX[m_VerticalLineIndex],dRangeY[MAX]));
	pDC->SelectObject(old);
	delete pen ;
	//----------------------------
...
/////////////////////////////////////////////////////////////////////////////
BOOL	CChart::SetVLineIndex(int vLineIndex)
{
	m_VerticalLineIndex = vLineIndex;
	return TRUE ;
}
//////////////////////////////////////////////////
// Set chart title
